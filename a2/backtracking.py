...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nxtvar.curDomain():
        ...
        for cnstr in csp.constraintsOf(nxtvar):
            if cnstr.numUnassigned() == 1:
                pruned.append(val)
                FC_result = FCCheck(cnstr, nxtvar, val)
                if FC_result == "DWO":
                    constraintsOK = False
                    if trace: print("<==falsified constraint\n")
                    break
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    while constraints:
        constraint = constraints.pop()  # make cnstr GAC
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():  # no more unassigned variables
        ...
        return [soln]# continue search to print all solutions
    bt_search.nodesExplored += 1
    #print(bt_search.nodesExplored)
    solns = []
    nxtvar = unAssignedVars.extract()  # select next variable to assign
    for val in nxtvar.curDomain():  # current domain
        nxtvar.setValue(val)
        constraintsOK = True
        ...
        if constraintsOK:
            ...
            if len(solns) > 0 and not allSolutions:
                ...
                # as we found a soln.
        for var in csp.variables():
            var.restoreValues(nxtvar, val) # restore values pruned by this assigment
    nxtvar.unAssign() # undo assigment to nxtvar
    unAssignedVars.insert(nxtvar)  # restore var to unAssignedVars
    return solns
