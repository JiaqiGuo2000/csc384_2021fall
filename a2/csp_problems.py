...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint = AllDiffConstraint("row_" + str(row_i), row)
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint = AllDiffConstraint("col_" + str(colj + 1), scope)
            constraint_list.append(constraint)
            #util.raiseNotDefined()
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint = AllDiffConstraint("col_" + str(cell_i + 1), scope)
                constraint_list.append(constraint)
                cell_i += 1
                #util.raiseNotDefined()
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for plane in planes_problem.planes:
        single_plane_variables = []
        can_fly = planes_problem.can_fly(plane)
        connected_vars = []
        for sequence_index in range(len(can_fly)):
            domain = ["unscheduled"]
            if sequence_index != 0:
                domain.extend(planes_problem.can_fly(plane))
            else:
                domain.extend(planes_problem.can_start(plane))
            new_var = Variable(str(sequence_index) + " schedule for " + plane, domain)
            connected_vars.append(new_var)
            if len(connected_vars) == 2:
                constraint_list.append(ConnectionConstraint(str(sequence_index - 1) + " " + str(sequence_index), connected_vars, planes_problem.can_follow))
                connected_vars.pop(0)

            variable_list.append(new_var)
            single_plane_variables.append(new_var)

    #for flight in planes_problem.flights:
    #    constraint_list.append( NValuesConstraint(flight + "plane scheduled", variable_list, [flight], 1, 1))
    constraint_list.append(AllDiffConstraint("Different schedule", variable_list))
    constraint_list.append(NValuesConstraint("all scheduled", variable_list, planes_problem.flights, len(planes_problem.flights), len(planes_problem.flights)))

    csp = CSP("Plane Problem", variable_list, constraint_list) #set to to your CSP
    ...
    for solution in solutions:
        ...
        curr_index = 0
        for plane in planes_problem.planes:
            format_solution.append([plane])
            can_fly = planes_problem.can_fly(plane)
            for sequence_index in range(len(can_fly)):
                format_solution[-1].append(solution[curr_index][1])
                curr_index += 1
        pre_ret.append(format_solution)
    ...
    for solution in pre_ret:
        ...
        for plane in solution:
            ...
            for flight in plane[1:]:
                if flight in planes_problem.maintenance_flights:
                    curr_distance = 0
                else:
                    curr_distance += 1
                if curr_distance >= planes_problem.min_maintenance_frequency:
                    accept = False
                    break
            if not accept:
                break
