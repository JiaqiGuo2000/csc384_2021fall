...
class TableConstraint(Constraint):
    ...
    def __init__(self, name, scope, satisfyingAssignments):
        '''Init by specifying a name and a set variables the constraint is over.
           ...
           Each satisfyt is itself a list, of length equal to
           the number of variables iing assignmenn the constraints scope.
...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        sa = []
        for i_position in range(8):
            for j_position in range(8):
                if i_position - j_position != 0 and abs(i_position - j_position) != abs(i - j):
                    sa.append([i_position + 1, j_position + 1])
...
class AllDiffConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        if self.scope()[0].getValue() == "unscheduled":
            return True
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for val in assignments:
            if val in self._required:
                count += 1
        return count <= self._ub and count >= self._lb
    ...
    def hasSupport(self, var, val):
        ...
        for other_var in self.scope():
            ...
            has_candidate = 0
            only_candidate = 1
            for possible_val in other_var.curDomain():
                if possible_val in self._required:
                    has_candidate = 1
                else:
                    only_candidate = 0
            max_count += has_candidate
            min_count += only_candidate
        #var.unAssign()
        return min_count <= self._ub and max_count >= self._lb
        #util.raiseNotDefined()


class ConnectionConstraint(Constraint):
    '''
    check if a plane is flying according to the given connection maps
    '''

    def __init__(self, name, scope, flight_map):
        Constraint.__init__(self,name, scope)
        self._name = "connection_" + name
        self._flight_map = flight_map
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.isAssigned():
                ...
                return True
        return (assignments[0], assignments[1]) in self._flight_map
    ...
    def hasSupport(self, var, val):
        ...
        if var == self.scope()[0]:
            ...
            for connection in self._flight_map:
                if connection[0] == start and connection[1] in end.curDomain():
                    return True
        else:
            start = self.scope()[0]
            end = val
            for connection in self._flight_map:
                if connection[1] == end and connection[0] in start.curDomain():
                    return True
        return False
